...
export KBUILD_BUILD_USER="Priyabrat"
export KBUILD_BUILD_HOST="neutrino"
export USE_CCACHE=1
BUILD_DIR=$KERNEL_DIR/build
VERSION="Plus"
DATE=$(date -u +%Y%m%d-%H%M)
ZIP_NAME=Legendary-$DEVICE-$VERSION-$DATE
...
if [ -x "$(command -v gdrive)" ]; then
	...
	echo -e "> Uploading Kernel zip to Google Drive..."
                ...
                echo -e "> Kernel zip uploaded succesfully"
                echo -e "> Check your Google Drive for the kernel zip mentioned above"
	fi
